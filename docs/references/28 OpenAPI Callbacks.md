...
In this case, you could want to document how that external API _should_ look. What _path operation_ it should have, what body it should expect, what response it should return, etc.
...
So, what we will do next is add the code to document how that _external API_ should look to receive the callback from _your API_.
...
This code won't be executed in your app, we only need it to _document_ how that _external API_ should look.

But, you already know how to easily create automatic documentation for an API with **FastAPI**.

So we are going to use that same knowledge to document how the _external API_ should look... by creating the _path operation(s)_ that the external API should implement (the ones your API will call).
...
Now use the parameter `callbacks` in _your API's path operation decorator_ to pass the attribute `.routes` from that callback router:
...
You will see your docs including a "Callbacks" section for your _path operation_ that shows how the _external API_ should look:
